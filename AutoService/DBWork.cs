...
namespace AutoService
{
    internal class DBWork
    {
        ...
        static public bool MakeDB(string _dbname = "AutoService.db")
        {
            ...
            string alter_table_mechanics = "ALTER TABLE Mechanics " + //ДЗ
                "ADD COLUMN " +
                "PaymentCoefficient " +
                "REAL DEFAULT 1.0;";
        ...
        private void AddMechanic(string name, double paymentCoefficient) //добавление мастера ДЗ
        {
            using (SQLiteConnection conn = new SQLiteConnection(path))
            {
                conn.Open();
                string query = "INSERT INTO Mechanics " +
                    "(Name, PaymentCoefficient) " +
                    "VALUES " +
                    "(@name, @paymentCoefficient)";
                SQLiteCommand cmd = new SQLiteCommand(query, conn);
                cmd.Parameters.AddWithValue("@name", name);
                cmd.Parameters.AddWithValue("@paymentCoefficient", paymentCoefficient);
                cmd.ExecuteNonQuery();
            }
        }

        private void EditMechanic(string oldName, string newName, double paymentCoefficient) //редактирование мастера ДЗ
        {
            using (SQLiteConnection conn = new SQLiteConnection(path))
            {
                conn.Open();
                string query = "UPDATE Mechanics SET " +
                    "Name = @newName, " +
        ...
        private void DeleteMechanic(string name) //удалить мастера ДЗ
        {
            using (SQLiteConnection conn = new SQLiteConnection(path))
            {
                conn.Open();
                string query = "DELETE FROM Mechanics WHERE Name = @name";
                SQLiteCommand cmd = new SQLiteCommand(query, conn);
                cmd.Parameters.AddWithValue("@name", name);
                cmd.ExecuteNonQuery();
            }
